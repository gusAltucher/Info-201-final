...
#Change echo to change how code is displayed (I think)
...
fullCrimeFreq %>% 
  mutate(sZipcode=str_replace_all(Zipcode,"981",":")) %>% 
  ggplot(aes(factor(sZipcode),Criminal.instances,fill=Criminal.instances))+
  geom_col()+
  scale_fill_viridis(option = "inferno")+
      labs(
    x= "Zip Code (Written As Last Two Digits)",
    y="Instances of Criminal Activity",
    fill = "Criminal Activity",
    title= "Instances of Criminal Activity in Each Zip Code"
  )
...
fullData %>% 
  ...
  ggplot(aes(factor(sZipcode),Income,fill=Income))+
  geom_col()+
        labs(
    x= "Zip Code (Written As Last Two Digits)",
...
fullCrimeFreq %>% 
  ...
  labs(
    x="Median Income",
    y="Instances of Criminal Activity",
    title="Insances of Criminal Activity Compared to Median Income"
  )
```
